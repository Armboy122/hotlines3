...
import { Picker, Toast, Dialog, ConfigProvider } from "antd-mobile";
import thTH from "antd-mobile/es/locales/th-TH";
import { createTaskDaily } from "@/lib/actions/task-daily";
...
import { INITIAL_FORM_STATE, type FormProps, type FormData } from "./types";
...
// ========== Main Form Component ==========
export default function AntdForm({ jobTypes, jobDetails, feeders, teams }: FormProps) {
  // ===== State =====
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      const submitData: CreateTaskDailyData = {
        ...
        urlsBefore: form.urlsBefore,
        urlsAfter: form.urlsAfter,
        latitude: form.latitude,
        longitude: form.longitude,
      };

      const result = await createTaskDaily(submitData);

      if (result.success) {
        Toast.show({ content: "บันทึกข้อมูลสำเร็จ", icon: "success" });
        resetForm();
      } else {
        Toast.show({ content: result.error || "เกิดข้อผิดพลาด", icon: "fail" });
      }
    } catch (error) {
      console.error("Submit error:", error);
      Toast.show({ content: "เกิดข้อผิดพลาดในการบันทึก", icon: "fail" });
    } finally {
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!validation.isValid) {
      Toast.show({ content: validation.message!, icon: "fail" });
      return;
    }
          ...
          <div className="space-y-6">
            ...
            <SectionCard icon={<CameraIcon />} title="รูปภาพประกอบ" color="orange">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <ImageUploadBox
                  label="รูปก่อนทำงาน *"
                  images={form.urlsBefore}
                  onAdd={(url) => updateForm("urlsBefore", [...form.urlsBefore, url])}
                  onRemove={(i) => updateForm("urlsBefore", form.urlsBefore.filter((_, idx) => idx !== i))}
                  color="emerald"
                />
                <ImageUploadBox
                  label="รูปหลังทำงาน"
                  images={form.urlsAfter}
                  onAdd={(url) => updateForm("urlsAfter", [...form.urlsAfter, url])}
                  onRemove={(i) => updateForm("urlsAfter", form.urlsAfter.filter((_, idx) => idx !== i))}
                  color="blue"
                />
              </div>
            </SectionCard>
            ...
            <SubmitButton
              ...
              disabled={isSubmitting || form.urlsBefore.length === 0}
...
function FormHeader() {
  return (
    <div className="relative bg-gradient-to-r from-emerald-500 via-emerald-600 to-teal-600 rounded-2xl p-6 mb-6 text-white shadow-xl overflow-hidden">
      ...
      <p className="text-center text-emerald-100 text-sm mt-1 relative z-10">(antd-mobile version)</p>
    </div>
  );
}
