...
import { getPlanConductors, deletePlanConductor, markPlanConductorDone, cancelPlanConductor, getPlanConductorsOverview } from '@/lib/actions/plan-conductor'
import PlanConductorForm from '@/components/forms/plan-conductor-form'

interface PlanConductor {
  id: bigint
  year: number
...
export default function PlanConductorsPage() {
  const [planConductors, setPlanConductors] = useState<PlanConductor[]>([])
  ...
  const [editingItem, setEditingItem] = useState<PlanConductor | null>(null)
  ...
  const loadData = async (year?: number) => {
    ...
    try {
      const [planResult, overviewResult] = await Promise.all([
        getPlanConductors(year),
        getPlanConductorsOverview(year)
      ])

      if (planResult.success && planResult.data) {
        setPlanConductors(planResult.data)
      }
  ...
  // Filter data
  const filteredData = planConductors.filter(item => {
    const matchesSearch = 
  ...
  const handleDelete = async (id: bigint) => {
    ...
    const result = await deletePlanConductor(id.toString())
    if (result.success) {
      loadData(selectedYear)
    } else {
  ...
  const handleMarkDone = async (id: bigint) => {
    const result = await markPlanConductorDone(id.toString())
    if (result.success) {
      loadData(selectedYear)
    } else {
      alert(result.error || 'เกิดข้อผิดพลาด')
  ...
  const handleCancel = async (id: bigint) => {
    ...
    const result = await cancelPlanConductor(id.toString())
    if (result.success) {
      loadData(selectedYear)
    } else {
      alert(result.error || 'เกิดข้อผิดพลาด')
  ...
  const exportData = () => {
    const csvContent = [
      ['ปี', 'การไฟฟ้า', 'ชื่อเต็ม', 'จุดรวมงาน', 'รายละเอียด', 'สถานะ', 'วันที่เสร็จ'].join(','),
    ...
    link.setAttribute('download', `แผนบำรุงรักษาไม้ฉนวน_${selectedYear}.csv`)
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  }

  const getStatusBadge = (item: PlanConductor) => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold">แผนบำรุงรักษาไม้ฉนวน</h1>
          <p className="text-muted-foreground">จัดการแผนงานบำรุงรักษาไม้ฉนวนของการไฟฟ้าต่างๆ</p>
        </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>เพิ่มแผนบำรุงรักษาไม้ฉนวน</DialogTitle>
          </DialogHeader>
          <PlanConductorForm
      ...
      <Dialog open={!!editingItem} onOpenChange={(open) => !open && setEditingItem(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>แก้ไขแผนบำรุงรักษาไม้ฉนวน</DialogTitle>
          </DialogHeader>
          {editingItem && (
            <PlanConductorForm
              initialData={{
