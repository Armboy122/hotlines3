...
const groupTasksByTeam = (tasks: ReturnType<typeof serializeTaskDaily>[]) =>
  tasks.reduce((acc, task) => {
    ...
    return acc
  }, {} as Record<string, { team: { id: string; name: string }; tasks: ReturnType<typeof serializeTaskDaily>[] }>)
...
export const tasksRoutes = new Elysia({ prefix: '/tasks' })
  ...
  .post('/', async ({ body, set }) => {
    try {
      const task = await taskDailyService.create(body as unknown as Parameters<typeof taskDailyService.create>[0])
      return { success: true, data: serializeTaskDaily(task) }
    } catch {
      set.status = 500
  ...
  .put('/:id', async ({ params, body, set }) => {
    try {
      const task = await taskDailyService.update({ ...(body as Record<string, unknown>), id: params.id } as Parameters<typeof taskDailyService.update>[0])
      return { success: true, data: serializeTaskDaily(task) }
    } catch {
      set.status = 500
